...
unsigned int ADC_getval(void)
{
    ...
    /*
    tmpval = tmpval - 25; // Calibrate the lighting (use a linear calibration based on debugging max and ADCgetval values)
    if (tmpval<0) {tmpval=0;} // If value of signed int drops below zero (negative) set value to zero!
    */
